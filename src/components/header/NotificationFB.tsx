        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              onClick={() => {
                ...
                setNewId(preguntas.filter((pregunta) => !pregunta.pregunta_seen).map((pregunta) => pregunta.pregunta_id));
                handleOnClick();
              }}
              variant="outline"
              size="icon"
          ...
          <PopoverContent className="p-0">
            <Card className="shadow-none border-0">
              ...
              <CardContent
                className={`p-4 max-h-[430px] overflow-y-scroll
                  ${preguntas.length < 3 && 'scrollbar-hide max-h-[380px]'}`}
              >
                {preguntas
                  .slice(0, cantNotiShown)
                  .map((pregunta) => (
                    <div
                      key={pregunta.pregunta_id}
                      className={`mb-4 rounded-md p-2 grid grid-cols-[25px_1fr] ${newId.includes(pregunta.pregunta_id) && 'bg-zinc-200'} items-start pb-4 last:mb-0 last:pb-0`}
                    >
