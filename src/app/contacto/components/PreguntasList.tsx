...
function PreguntasList({ preguntas }: { preguntas: Preguntas }) {
  ...
  return (
    <div className="">
      <h2 className="my-10 text-3xl font-semibold">Consultas por responder: {cantidadPreguntasSinRespuesta}</h2>
      {preguntas.map((pregunta) => (
        <div
          className="mb-4 grid grid-cols-1 gap-6 border-b-2 pb-4 sm:grid-cols-2 md:grid-cols-3"
          key={pregunta.pregunta_id}
        >
          <h2># {pregunta.pregunta_id}</h2>
          <div>
            <h2 className="mb-4 text-lg font-semibold">Consulta</h2>
            <h3 className="text-lg font-normal">{pregunta.pregunta_name}</h3>
            <p className="text-foreground">{pregunta.pregunta_email}</p>
            <p>
              Teléfono: {pregunta.pregunta_telefono.slice(0, 4)} {pregunta.pregunta_telefono.slice(4, 8)}{' '}
            </p>
            ...
            <p className="text-foreground">
              {pregunta.pregunta_created_at.toLocaleString('es-ES', {
            ...
            <Button
              onClick={() => deletePregunta(pregunta.pregunta_id)}
              className="mt-4 flex gap-2 bg-rose-600 hover:bg-rose-500"
            >
          ...
          <div>
            <h2 className="mb-4 text-lg font-semibold">Respuesta</h2>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center">
                  <Button
                    ...
                    onClick={() => {
                      handleOnClick(pregunta), setId(pregunta.pregunta_id);
                    }}
                  >
                    {clicked === true && id === pregunta.pregunta_id ? (
                  ...
                  {clicked === true && id === pregunta.pregunta_id && (
                    <Button className="ml-1 bg-green-600 hover:bg-emerald-500" onClick={() => createRespuesta()}>
                      Responder
                    </Button>
                  )}
                </div>
